  ...
  public:
    ...
    bool isOnAnyBody(int x, int y) {
      for (int pi = 0; pi < 2; ++pi) {
        for (int i = 0; i < p[pi].length; ++i) {
          if (p[pi].bodyX[i] == x && p[pi].bodyY[i] == y) return true;
        }
      }
      return false;
    }
    ...
    bool checkSelfCollision(int pi) {
      for (int i = 1; i < p[pi].length; ++i) {
        if (p[pi].bodyX[i] == p[pi].headX && p[pi].bodyY[i] == p[pi].headY) return true;
      }
      return false;
    }
    ...
    bool checkCollisionWithOther(int me) {
      ...
      for (int i = 0; i < p[other].length; ++i) {
        if (p[other].bodyX[i] == p[me].headX && p[other].bodyY[i] == p[me].headY) return true;
      }
      return false;
    }
    ...
    void respawnPlayer(int pi) {
      ...
      if (p[pi].headX == foodX && p[pi].headY == foodY) spawnFood();
    }
    ...
    bool update(ControllerPtr myControllers[]) {
      ...
      for (int pi = 0; pi < 2; ++pi) {
        ...
        if ((dpad & GAME_DPAD_UP) || axisY < -200) { p[pi].nextDirX = 0; p[pi].nextDirY = -1; }
        if ((dpad & GAME_DPAD_DOWN) || axisY > 200) { p[pi].nextDirX = 0; p[pi].nextDirY = 1; }
        if (axisX < -200) { p[pi].nextDirX = -1; p[pi].nextDirY = 0; }
        if (axisX > 200) { p[pi].nextDirX = 1; p[pi].nextDirY = 0; }
        ...
        if (pi == 0) {
          ...
          if (now - lastSpeedChangeTime > 150) {
            ...
            if (btns & 0x04) { speedMultiplier = max(0.3f, speedMultiplier - 0.1f); lastSpeedChangeTime = now; drawHud(); }
            if (btns & 0x08) { speedMultiplier = min(2.0f, speedMultiplier + 0.1f); lastSpeedChangeTime = now; drawHud(); }
          }
        }
      }
      ...
      if (now - lastMoveTime > currentDelay) {
        ...
        for (int pi = 0; pi < 2; ++pi) {
          ...
          if (p[pi].headX < 0 || p[pi].headX >= GRID_WIDTH || p[pi].headY < 0 || p[pi].headY >= GRID_HEIGHT) {
            ...
            p[pi].lives--;
            p[pi].alive = false;
            if (p[pi].lives > 0) respawnPlayer(pi);
            continue;
          }
        ...
        if (p[0].lives <= 0 && p[1].lives > 0) return false;
        if (p[1].lives <= 0 && p[0].lives > 0) return false;
        if (p[0].lives <= 0 && p[1].lives <= 0) return false; // both dead -> end
      }
...
void Snake_init(ControllerPtr P1, ControllerPtr P2) {
